...
public class ali extends CordovaPlugin {
    ...
    public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
        ...
        if (action.equals("isAliPayInstalled")) {
            this.isAliPayInstalled(callbackContext);
            return true;
        }
        return false;
    }
    ...
    private void isAliPayInstalled(final CallbackContext callbackContext) {
        Uri uri = Uri.parse("alipays://platformapi/startApp");
        Intent intent = new Intent(Intent.ACTION_VIEW, uri);
        ComponentName cName = intent.resolveActivity(cordova.getActivity().getPackageManager());
        if (cName == null) callbackContext.success(0);
